    ...
    >
      <div className="flex gap-x-1 pb-8 max-md:pb-6">
        {Array.from({ length: test.rating }).map((_, i) => (
          <Star alt="Star Rating Icon" className="fill-gray-50" key={i} />
        ))}
        {Array.from({ length: 5 - test.rating }).map((_, i) => (
          <Star alt="Star Rating Icon" key={i} />
        ))}
      </div>
      <p className="text-gray-50 pb-16 text-lg/loose font-light max-xl:text-base/loose max-md:pb-12">
        {test.description}
      </p>
      <div className="flex items-center gap-x-6 max-xl:gap-x-4">
        ...
        />
        <div>
          <p className="text-primary-50 text-xl/7 font-bold tracking-tight max-xl:text-lg/8">
            {test.name}
          </p>
          <p className="text-gray-100 text-base/loose tracking-tight max-xl:text-sm">
            {test.title}
          </p>
        </div>
      </div>
